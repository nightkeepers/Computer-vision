...
Hough::Hough(char filename[], int mode) {
...
void Hough::findTopK()
{
	...
	vector<node> points2;
	for (int i = points.size() - 1; i > points.size() - 22; i--)
	{
...
CImg<unsigned char> Hough::drawLine()
{
	...
	for (i = 0; i < points.size(); i++)
	{
		...
		{
			//cout << points[i].x << " " << points[i].y << endl;
			if ((abs((ks[j]-k)/k)<0.01||abs(thetas[j]-theta)<5)&&abs((bs[j] - b) / b)<0.01)
				flag = 1;
		}
		if (flag)
			continue;
